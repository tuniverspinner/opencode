...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.CYF_ZED_DB
  process.env.CYF_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.CYF_ZED_DB
    else process.env.CYF_ZED_DB = previous
    home.mockRestore()
  }
})
