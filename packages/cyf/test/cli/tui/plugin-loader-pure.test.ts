...
`,
  ...
  const pure = process.env.CYF_PURE
  const meta = process.env.CYF_PLUGIN_META_FILE
  process.env.CYF_PURE = "1"
  process.env.CYF_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.CYF_PURE
    } else {
      process.env.CYF_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.CYF_PLUGIN_META_FILE
    } else {
      process.env.CYF_PLUGIN_META_FILE = meta
    }
  }
})
