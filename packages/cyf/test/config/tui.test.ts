...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.CYF_CONFIG
  delete process.env.CYF_TUI_CONFIG
...
it.instance("project config takes precedence over CYF_TUI_CONFIG (matches CYF_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("CYF_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from CYF_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
