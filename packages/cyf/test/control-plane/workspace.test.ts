...
import { EventSequenceTable } from "@cyf-ai/core/event/sql"
...
import { WorkspaceV2 } from "@cyf-ai/core/workspace"
import { WorkspaceTable } from "@cyf-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  CYF_AUTH_CONTENT: process.env.CYF_AUTH_CONTENT,
  CYF_EXPERIMENTAL_WORKSPACES: process.env.CYF_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CYF_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CYF_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CYF_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CYF_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CYF_EXPERIMENTAL_WORKSPACES).toBe("true")
