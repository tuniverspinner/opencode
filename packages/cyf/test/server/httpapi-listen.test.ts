...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.CYF_SERVER_PASSWORD
  else process.env.CYF_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.CYF_SERVER_USERNAME
  else process.env.CYF_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.CYF_SERVER_PASSWORD = auth.password
  Flag.CYF_SERVER_USERNAME = auth.username
  process.env.CYF_SERVER_PASSWORD = auth.password
  process.env.CYF_SERVER_USERNAME = auth.username
