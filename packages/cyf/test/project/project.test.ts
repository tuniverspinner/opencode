...
import { Hash } from "@cyf-ai/core/util/hash"
...
import { AppProcess } from "@cyf-ai/core/process"
import { ProjectV2 } from "@cyf-ai/core/project"
import { ProjectCopy } from "@cyf-ai/core/project/copy"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const opencodeFile = path.join(tmp, ".git", "cyf")
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "cyf")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "cyf")
      const wrongCache = path.join(parentDir, ".git", "cyf")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "cyf")
      const cacheB = path.join(bareB, "cyf")
      const wrongCache = path.join(parentDir, ".git", "cyf")
