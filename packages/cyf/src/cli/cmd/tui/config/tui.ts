...
import { makeRuntime } from "@cyf-ai/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import * as Log from "@cyf-ai/core/util/log"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@cyf-ai/core/npm"
import type { DeepMutable } from "@cyf-ai/core/schema"
import type { TuiAttentionSoundName } from "@cyf-ai/plugin/tui"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and CYF_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CYF_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CYF_TUI_CONFIG override, if set.
  if (Flag.CYF_TUI_CONFIG) {
    const configFile = Flag.CYF_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and CYF_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.CYF_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== Flag.CYF_CONFIG_DIR) continue
