...
export const CYF_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CYF_MODE_KEY = "opencode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CYF_MODE_KEY, CYF_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CYF_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CYF_MODE_KEY, stack.at(-1)?.mode ?? CYF_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CYF_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CYF_MODE_KEY, undefined)
