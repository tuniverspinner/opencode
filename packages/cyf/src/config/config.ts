...
import { serviceUse } from "@cyf-ai/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@cyf-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@cyf-ai/core/v1/config/console-state"
...
import { ConfigV1 } from "@cyf-ai/core/v1/config/config"
import { ConfigPermissionV1 } from "@cyf-ai/core/v1/config/permission"
...
function globalConfigFile() {
  const candidates = ["cyf.jsonc", "cyf.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.CYF_CONFIG && !Flag.CYF_CONFIG_DIR && !Flag.CYF_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "cyf.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "cyf.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CYF_CONFIG_CONTENT") return "local"
        ...
        if (Flag.CYF_CONFIG) {
          yield* merge(Flag.CYF_CONFIG, yield* loadFile(Flag.CYF_CONFIG, authEnv))
          log.debug("loaded custom config", { path: Flag.CYF_CONFIG })
        }
        ...
        if (Flag.CYF_CONFIG_DIR) {
          log.debug("loading config from CYF_CONFIG_DIR", { path: Flag.CYF_CONFIG_DIR })
        }
        ...
        if (process.env.CYF_CONFIG_CONTENT) {
          const source = "CYF_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CYF_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from CYF_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CYF_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CYF_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.CYF_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CYF_PERMISSION))
          } catch (err) {
            log.warn("CYF_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.CYF_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.CYF_DISABLE_PRUNE) {
